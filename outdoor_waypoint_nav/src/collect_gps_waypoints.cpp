...
// custom keyboard subscriber callback
void key_CB(const sensor_msgs::Joy joy_msg)
{
	if(joy_msg.buttons[collect_button_num]==1)
	{
		collect_request = true;
...
int main(int argc, char** argv)
{
    ...
    //Initiate subscribers
		...
		// added custom code to subscribe to the keyboard for control inputs
		//ros::Subscriber sub_key = n.subscribe("/husky_velocity_controller/cmd_vel", 10, key_CB);
	...
	// Give instructions:
	// HOW IT WORKS
	// Anytime the collect_button/key is pressed (which triggers the collect_request to true), 
	//	The code checks to see if the robot has moved enough distance 
	//		if yes: the new coordinate is written out to the waypoint file
	//		if no: it prints out a message that the robot has not covered enough distance for waypoint collection
